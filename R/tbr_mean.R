...
#' @export
tbr_mean <- function(.tbl, x, tcolumn, unit = "years", n, conf.level = 0.95, ...) {
...
#' @return list
#' @keywords internal
tbr_mean_window <- function(x, tcolumn, unit = "years", n, conf.level, i, ...) {
  ...
  results <- tibble::as_tibble(as.list(DescTools::MeanCI(x = temp, conf.level = conf.level, ...)))
