...
Dissolved_Oxygen %>%
  ...
  group_by(Station_ID) %>%
  tbr_mean(Average_DO, Date, "years", 5) %>%
  ggplot() +
  geom_step(aes(Date, mean, color = Station_ID)) +
  geom_ribbon(aes(Date, ymin = lwr.ci, ymax = upr.ci, fill = Station_ID), alpha = 0.5)
```
...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
df %>% 
  tbr_gmean(y, time, unit = "minutes", n = 120, method = "classic") %>%
  ggplot() +
...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
df %>% 
  tbr_gmean(y, time, unit = "hours", n = 12, method = "classic") %>%
  ggplot() +
  geom_point(aes(time, y), alpha = 0.25) +
  geom_step(aes(time, mean)) +
  geom_ribbon(aes(time, ymin = lwr.ci, ymax = upr.ci), alpha = 0.5, stat = "stepribbon") +
...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
df %>% 
  tbr_gmean(y, time, unit = "days", n = 1, method = "classic") %>%
  ggplot() +
  geom_point(aes(time, y), alpha = 0.25) +
  geom_step(aes(time, mean)) +
  geom_ribbon(aes(time, ymin = lwr.ci, ymax = upr.ci), alpha = 0.5, stat = "stepribbon") +
...
`tbr_mean` and `tbr_gmean` default to a bootstrap approach for calculating confidence intervals. Computations can be substantially decreased by using `method = "classic"`. 
